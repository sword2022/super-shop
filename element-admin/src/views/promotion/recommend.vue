<template>
  ...
  >
    ...
    >
      <el-form ref="settingsForm" :model="settingsForm" :inline="false">
        <el-form-item :label="`首页展示${pageTitle}`">
          <el-switch v-model="settingsForm.show_recommend"></el-switch>
        </el-form-item>
        <el-form-item>
    ...
    >
      ...
      <el-table-column prop="parent.name" label="品名"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async dialogFormSubmit() {
      if (this.editting) {
        let res = await this.$http.put(`recommend/edit/${this.dialogForm._id}`, this.dialogForm);
        ...
        let res = await this.$http.post("recommend/create", this.dialogForm);
    ...
    async fetch() {
      ...
      //获取列表
      res = await this.$http.get("recommend/list");
      this.list = res.data;
    },
    async edit(row) {
      this.dialogVisible = true;
      let res = await this.$http.get(`recommend/edit/${row._id}`);
    ...
    async remove(row) {
      this.$confirm('是否删除"' + row.parent.name + '"?', "提示", {
        ...
        //请求 删除分类接口
        let res = await this.$http.delete(`recommend/delete/${row._id}`);
        window.console.log(res);
        this.$message({
          type: "success",
          message: "删除成功!",
